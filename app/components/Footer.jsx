        ...
        {(footer) =>
          footer ? (
            <footer className="footer">
              ...
              <div className="footer-map">
                <img
                  src="https://cdn.shopify.com/s/files/1/0584/8688/2388/files/map.webp?v=1734591783"
                  alt="Address"
                />
              </div>
...
function FooterMenu({ menu, primaryDomainUrl, publicStoreDomain }) {
  return (
    <nav className="footer-menu" role="navigation">
      {(menu?.items || []).map((item, index) => {
        if (!item?.url) return null; // Prevent errors if item or URL is undefined

        let customUrl = item.url;

        // Modify URLs based on item index
        if (index === 2) customUrl = "/faq"; // 3rd child
        if (index === 3) customUrl = "/contact"; // 4th child
        if (index === 5) customUrl = "/segate-customers"; // 6th child
...
/**
 * Footer Logo and Address Component
 */
function FooterLogoAddress({ logoUrl, address }) {
  return (
    <div className="footer-logo-address text-white">
      ...
      <div className="footer-logo">
        <a href="/">
          <img src={logoUrl} alt="Company Logo" className="footer-logo-image" />
          <h4 className="footer-address-title text-white font-medium text-lg tracking-widest py-6">
            ATP DATA SERVICES
          </h4>
        </a>
      </div>
      ...
      <div className="footer-address">
        <p className="footer-address-text text-white text-sm">{address}</p>
        <a href="#" className="foot-email text-sm">
          <span>info@dataservices.com</span>
        </a>
      </div>
    </div>
  );
